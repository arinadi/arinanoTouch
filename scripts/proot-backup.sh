...
# ⚠️  MANUAL TOOL — only run BEFORE updating arinanoTouch
# Usage:
#   1. bash ~/.arinanotouch/scripts/proot-backup.sh
#   2. bash ~/update.sh  (fresh install)
#   3. bash ~/.arinanotouch/scripts/proot-restore.sh
#   4. bash ~/.arinanotouch/scripts/patch.sh  (reinstall your packages)
...
BACKUP_DIR="$HOME/.arinanotouch/backups"
